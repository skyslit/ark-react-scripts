...
module.exports = function (webpackEnv, useServer = false) {
  webpackEnv = useServer === true ? 'production' : webpackEnv;
  ...
  if (isModuleDevelopment) {
    useServer = false;
    virtualModules = new VirtualModulesPlugin({
      [paths.appHtml]: fs.readFileSync(path.join(__dirname, './../module-dev/assets/index.html')),
  ...
  function getConfig(target) {
    ...
    return {
      ...
      entry: [
        ...
        (function () {
          if (isModuleDevelopment) {
            return path.join(process.cwd(), 'src/index.tsx');
          } else if (isServer) {
            return paths.appServerJs;
          }
      ...
      output: {
        ...
        chunkFilename: (function () {
          if (isServer) {
            // On server, no need for JS chunk files, should be good as a single file
            return undefined;
          }
